function pad(n, width, z) {
	...
	//Parse.initialize("PkcoUNsE8PqGAdFXIU7F4fNGTD84lQWsRsQGl3Ft", "E4PynSAlb7AnhAjr0266wMbr5otaSrgRRUtuh2wK");
        ...
        if ($("#frm_login").validationEngine('validate'))
        {
            ...
            Parse.User.logIn($('#email').val(), $('#password').val(), {
                success: function (user) {
                    ...
                    var currentUser = Parse.User.current();
                    var now = new Date();
                    var expiredate = new Date(user.attributes.expire_date);

                    var expireday = ("0" + expiredate.getDate()).slice(-2);
                    var expiremonth = ("0" + (expiredate.getMonth() + 1)).slice(-2);
                    var expire = expiredate.getFullYear() + "-" + (expiremonth) + "-" + (expireday);
                    ...
                    //alert("Expire Date is: " + expire);
                    //if (currentUser && expire >= today) {
                    if (currentUser && today <= expire) {
    ...
    $("#addListing").click(function () {
        ...
        if ($("#frm_addListing").validationEngine('validate'))
        {
            ...
            if ($('#todo_task').val()) {
                ...
                todo.set("duetime", $('#todo_duetime').val());
    ...
    if (window.location.hash) {
        ...
        query.get(hash, {
            success: function (todo) {
                ...
                $("#duedate").val(today);
                $("#duetime").val(todo.attributes.duetime);
        ...
        success: function (todos) {
            if (todos.length > 0)
            {
                $.each(todos, function (i, todo) {
                    ...
                    var today = (month) + "/" + (day) + "/" + now.getUTCFullYear();
