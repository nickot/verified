...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	this.sendTwitterImage = function(options) {
		...
		message.parameters.push(["oauth_version", "1.1"]);

		// alert(' this.sendTwitterImage : message.parameters > ' + message.parameters + '\n\n accessor : ' + accessor );
		//
		// alert(accessor)
		...
		client = Ti.Network.createHTTPClient({
			// SSL : true,
			// tlsVersion :  Titanium.Network.TLS_VERSION_1_2,
			// status : 'aaa',
			// media : [{
		...
		// alert(accessTokenStore[pService] + ' - ' + pService );
		// alert(Ti.App.Properties.getString("Social.js-AccessToken-" + pService));
		// alert("accessTokenStore[pService] : " + accessTokenStore[pService] + "\nSocial.js-AccessToken- : " + ( Ti.App.Properties.getString("Social.js-AccessToken-" + pService)) + '\naccessToken' + accessToken + '\naccessTokenSecret' + accessToken)
	...
	this.showLoadingUI = function() {
		...
		Ti.API.info(' this.send : options > ' + options + '\n this.send : callback > ' + callback);
		...
		// alert(' this.send : message.parameters > ' + message.parameters)
		// alert(options)
		...
		var parameterMap = OAuth.getParameterMap(message.parameters), client = Ti.Network.createHTTPClient({
			onload : function() {
				Ti.API.info(' this.send : onload : this.responseText > ' + this.responseText);
				client.status == 200 ? pSuccessMessage && pSuccessMessage(this.responseText) : pErrorMessage && pErrorMessage(this.responseText);
			},
			onerror : function() {
				Ti.API.info(' this.send : onerror : this.responseText > ' + this.responseText);
	...
	twitter : {
		...
		authorize : "https://api.twitter.com/oauth/authorize?force_login=true&",
	...
	return urls == null ? (alert("The Social.js module does not support " + site + " yet!"), null) : {
			...
			if (!adapter.isAuthorized()) {
				function receivePin() {
					...
					adapter.getAccessToken(urls.accessToken, function(evt) {
						...
						evt.success ? (adapter.saveAccessToken(site), callback && callback()) : showDefaultAlert("Did not get access token now!");
					});
				}


				adapter.showLoadingUI(), adapter.getRequestToken(urls.requestToken, function(evt) {
					evt.success ? adapter.showAuthorizeUI(urls.authorize + evt.token, receivePin) : showDefaultAlert("Did not get access token now!");
				});
			} else
				callback && callback();
		},
		shareImage : function(options) {
			...
			this.authorize(function() {
				// alert( "shareImage : options > " + options)
